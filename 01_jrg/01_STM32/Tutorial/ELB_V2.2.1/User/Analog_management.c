...
#endif
#ifdef HW_CONSOLE
	  GPIO_InitStruct.Pin = ADC_1.Pin | ADC_2.Pin | ADC_3.Pin;	
  HAL_GPIO_Init(ADC_1.Port, &GPIO_InitStruct);
#endif
#ifdef HW_R5_ELB
	  GPIO_InitStruct.Pin = ADC_1.Pin | ADC_2.Pin;	
  HAL_GPIO_Init(ADC_1.Port, &GPIO_InitStruct);
#endif	
...
#ifdef HW_ELB
 	osThreadFlagsSet(id_task_TR1,FLAG_ANA_CONFIG_READY);
 	osThreadFlagsSet(id_task_TR2,FLAG_ANA_CONFIG_READY);
 	osThreadFlagsSet(id_task_TR3,FLAG_ANA_CONFIG_READY);
#endif
#ifdef HW_R5_ELB
 	osThreadFlagsSet(id_task_TR_WF,FLAG_ANA_CONFIG_READY);
#endif
}
...
#ifdef HW_R5_ELB
  HAL_GPIO_DeInit(ADC_1.Port, ADC_1.Pin);
	HAL_GPIO_DeInit(ADC_2.Port, ADC_2.Pin);
#endif
...
void ANA_def(){
...
#ifdef HW_ELB
	ADC_1.Pin = GPIO_PIN_0;
	ADC_1.Port = GPIOA;
	ADC_1.ADC_channel = ADC_CHANNEL_0;
	ADC_1.ADC_Value = (uint16_t*)&ADC1Buffer[0];
	ADC_1.Sensor_type = Linear;
	ADC_1.Range_mV = 5000;
	...
	ADC_2.Pin = GPIO_PIN_1;
	ADC_2.Port = GPIOA;
	ADC_2.ADC_channel = ADC_CHANNEL_1;
	ADC_2.ADC_Value = (uint16_t*)&ADC1Buffer[1];
	ADC_2.Sensor_type = Linear;
	ADC_2.Range_mV = 5000;
	ADC_2.Filtered_Value = (uint16_t*)&Filter_Buffer[1][Filter_AV_NB];
